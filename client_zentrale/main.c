...
static void usage(char *name)
{
   ...
   printf("%s ([-v] [-f] -a <addr> -p <port> [-l path]) | -?\n", name);
   ...
   else
   {
      ...
      if (DoFork)
      {
         ...
         else if (ChildPid == 0)
         {
            ...
            {
               ZentraleInit(Zentrale, Verbose, IpAddr, Port, LokPath);
               ZentraleRun(Zentrale);
               ZentraleExit(Zentrale);
